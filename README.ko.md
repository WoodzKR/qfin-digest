...
한국어 리포트는 [humanize-korean](https://github.com/epoko77-ai/im-not-ai) 스킬로
한 번 더 윤문해서 번역체를 걷어낸다. `--review` 를 붙이면 먼저 비평 패스가 돈다 —
논문이 실제로 기대는 주장, 식별 가정, 타당성 위협, 가장 강한 반론을 뽑아 결과·한계
섹션에 반영한다. 두 스킬 모두 `~/.claude/skills` 에 설치돼 있다.
